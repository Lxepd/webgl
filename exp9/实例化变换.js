...
function animation(){
	...
	rotaY -= 3 * elapsed / 10.0;
	if(rotaY < -180.0)
		rotaY += 360.0;
	rotaX -= 3 * elapsed / 10.0;
	if(rotaX < -80.0)
		rotaX = -80.0;
	...
	tranX = Math.sqrt(Math.pow(ax, 2) - Math.pow(tranZ, 2) * Math.pow(ax, 2) / Math.pow(bz, 2)) * dirX;
	if(tranX == 0)
		dirX *= -1;
...
function render() {
	...
	drawCube(mult(translate(0, 0, 0), scale(3, yScale, 3)));

	gl.uniform4f(u_StartColor, 0, 0, 0, 1);
	gl.uniform4f(u_EndColor, 0, 0, 1, 1);
	drawCube(mult(translate(-10, 0, 0), mult(rotateX(thetaLeft),scale(2, 10, 10))));

	gl.uniform4f(u_StartColor, 0, 0, 0, 1);
	gl.uniform4f(u_EndColor, 0, 1, 0, 1);
	drawCube(mult(mult(translate(tranX, 0, tranZ), mult(rotateY(rotaY), rotateX(rotaX))), scale(15, 4, 4)));
