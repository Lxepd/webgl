...
function animation(){
	...
	if (runAnimation) {	// 如果动画开启
		...
		hourOfDayMoon += hours;
		dayOfYearMoon += hours / 5.0
		hourOfDayTree += hours;
		dayOfYearTree += hours / 10;
		hourOfDaytu += hours;
		dayOfYeartu += hours / 11.3;
		hourOfDayTW += hours;
		dayOfYearTW += hours / 17.2;
		hourOfDayHW += hours;
		dayOfYearHW += hours / 16;
...
function drawSolarSystem(mvp){
	...
	mvp = mult(mvp, scale(0.72, 0.72, 0.72)); // 控制太阳大小的缩放变换
	
	mvp = mult(mvp, rotateY(360.0 * dayOfYear * 8 / 365.0));
	mvp = mult(mvp, translate(-1.3, 0, 0));
	mvp = mult(mvp, rotateX(90));

	//mvp = mult(mvp, lookAt(vec3(1.5, 0, 0), vec3(0, 0, 0), vec3(0, 0, 0)));
	...
	//mvp = mult(mvp, lookAt(vec3(1, 0, 0), vec3(0, 0, 0), vec3(0, 1, 0)));
	...
	mvp = mult(mvp, translate(5.3, 0.0, 0.0));
	drawX(mvp);
	mvp = mvpStack.pop();

	//tree
	mvpStack.push(mvp);
	mvp = mult(mvp, rotateY(360.0 * dayOfYearTree / (365 * 12)))
	mvp = mult(mvp, translate(8.0, 0.0, 0.0));
	tree(mvp);
	mvp = mvpStack.pop();

	//tu
	mvpStack.push(mvp);
	mvp = mult(mvp, rotateY(360.0 * dayOfYeartu / (365 * 30)))
	mvp = mult(mvp, translate(11.2, 0.0, 0.0));
	tu(mvp);
	mvp = mvpStack.pop();

	//TW
	mvpStack.push(mvp);
	mvp = mult(mvp, rotateY(360.0 * dayOfYearTW / (365 * 84)))
	mvp = mult(mvp, translate(13.2, 0.0, 0.0));
	TW(mvp);
	mvp = mvpStack.pop();

	//HW
	mvpStack.push(mvp);
	mvp = mult(mvp, rotateY(360.0 * dayOfYearHW / (365 * 165)))
	mvp = mult(mvp, translate(15.2, 0.0, 0.0));
	HW(mvp);
	mvp = mvpStack.pop();
}
...
function drawEarthSystem(mvp){
	...
	mvp = mult(mvp, translate(0.8, 0.0, 0.0));
	drawMoon(mvp);
	mvp = mvpStack.pop();
}
...
function drawX(mvp){
	...
	mvp = mult(mvp, scale(0.5, 0.5, 0.5));
	mvp = mult(mvp, rotateX(90));

	gl.uniformMatrix4fv(u_MVPMatrix, false, flatten(mvp));
	gl.uniform3f(u_Color, 0.9, 0.5, 0.9);
	gl.drawArrays(gl.LINES, 0, numVertices);
	...
	mvp = mult(mvp, scale(0.08, 0.08, 0.08));

	gl.uniformMatrix4fv(u_MVPMatrix, false, flatten(mvp));
	gl.uniform3f(u_Color, 0.8, 0.2, 0.3);
	gl.drawArrays(gl.LINES, 0, numVertices);
	...
	mvp = mult(mvp, rotateY(-360.0 * 8.0 * dayOfYear / 300.0));
	mvp = mult(mvp, translate(0.95, 0, 0));
	mvp = mult(mvp, scale(0.08, 0.08, 0.08));

	gl.uniformMatrix4fv(u_MVPMatrix, false, flatten(mvp));
	gl.uniform3f(u_Color, 0.4, 0.4, 0.3);
	gl.drawArrays(gl.LINES, 0, numVertices);

	mvp = mvpStack.pop();
}

var hourOfDayMoon = 0.0;
var dayOfYearMoon = 0.0;
// the moon
function drawMoon(mvp){
	mvpStack.push(mvp);
	/*画月球*/
	// 用dayOfYear来控制其绕地球的旋转
	mvp = mult(mvp, rotateY(360.0 * hourOfDayMoon / 3.0));
	// 用于控制地月距离的平移变换
	...
	mvp = mult(mvp, rotateY(360.0 * dayOfYearMoon / 48.0))
	mvp = mult(mvp, translate(0.3, 0, 0));
	mvp = mult(mvp, scale(0.08, 0.08, 0.08));

	gl.uniformMatrix4fv(u_MVPMatrix, false, flatten(mvp)); // 传模视投影矩阵
	gl.uniform3f(u_Color, 0.6, 0.2, 0.6 );
	gl.drawArrays(gl.LINES, 0, numVertices);
...
function tree(mvp){
	...
	mvp = mult(mvp, rotateY(360.0 * hourOfDayTree / 10.0));
	mvp = mult(mvp, scale(0.9, 0.9, 0.9));
	mvp = mult(mvp, rotateX(90));
...
function tu(mvp){
	...
	mvp = mult(mvp, rotateY(360.0 * hourOfDaytu / 11.3));
	mvp = mult(mvp, scale(0.64, 0.64, 0.64));
	mvp = mult(mvp, rotateX(90));

	gl.uniformMatrix4fv(u_MVPMatrix, false, flatten(mvp)); // 传模视投影矩阵
	gl.uniform3f(u_Color, 0.8, 0.6, 0.4 );
	gl.drawArrays(gl.LINES, 0, numVertices);
...
function TW(mvp){
	...
	mvp = mult(mvp, rotateY(360.0 * hourOfDayTW / 17.));
	mvp = mult(mvp, scale(0.51, 0.51, 0.51));
	mvp = mult(mvp, rotateX(90));
...
function HW(mvp){
	...
	mvp = mult(mvp, rotateY(360.0 * hourOfDayHW / 16));
	mvp = mult(mvp, scale(0.58, 0.58, 0.58));
	mvp = mult(mvp, rotateX(90));

	gl.uniformMatrix4fv(u_MVPMatrix, false, flatten(mvp)); // 传模视投影矩阵
	gl.uniform3f(u_Color, 0.2, 0.9, 0.9 );
	gl.drawArrays(gl.LINES, 0, numVertices);
...
function render() {
	...
	mvp = mult(mvp, lookAt(vec3(0.0, 12.0, 40.0), vec3(.0, .0, .0), vec3(0, 1, 0)));
